...
#### 🚨 SUB-AGENT BOUNDARY & RESPONSIBILITY ISOLATION MATRIX
  ...
  - 🧪 **Tester Agent Role**:
    ...
    * Strict Boundary: You MUST permanently isolate the `target_component` path layout syntax based strictly on the target technology layer and test modality with zero cross-contamination:
        1. **BACKEND UNIT TEST WINDOW & DIRECTORY ISOLATION:** For regular modular Java unit tests (such as testing Controllers, Services, Repositories), you MUST strictly employ a two-element layout formatted precisely as the semicolon-separated pattern: `<production_source_file_path>;<corresponding_test_class_file_path>`. The second path MUST point to a valid JUnit test file (e.g., `./sources/backend/service-name/src/main/java/.../X.java;./sources/backend/service-name/src/test/java/.../XTest.java`). You are CRITICALLY BANNED from leaving a production Java file naked without its semicolon JUnit pair in a [Tester] log.
...
- **HARD-ANCHORED TEMPLATE RENDERING MATRIX:** When processing this active block, you MUST execute the output stream following the exact vertical layout lines provided below in a strict, unbreakable linear order:
    ...
    * Step 3: Mandatory Atomic Task Matrix Explosion Invariant: Parse the 4th column cell ("Architectural Component / Module Path") of your validated phase row matrix, splitting the raw data string primitive using `<br/>` as the strict delimiter. 
        - You MUST count the total number of physical file paths (let this integer be F) and the total number of assigned sub-agents contextually listed inside the 6th column of that active row (let this integer be S). You MUST programmatically expand and generate exactly (F * S) independent, standalone atomic sub-task nodes inside the daily logs, ensuring each row is wrapped perfectly between the structural boundaries `<!--ATOMIC_SUB_TASK_NODE_START-->` and `<!--ATOMIC_SUB_TASK_NODE_END-->`.
        - You ARE CRITICALLY BANNED from flattening multiple file paths into a single text block, or omitting any declared sub-agent layout signature found in that phase row (including infra-layer personas like [GCP] or documentation assets like .md). Missing a single file-agent execution pairing triggers an automatic framework deployment failure.
