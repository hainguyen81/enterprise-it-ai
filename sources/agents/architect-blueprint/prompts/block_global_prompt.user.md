...
{% if not force_full_export %}
<RULE>
- All the detailed phase logs generated in the `<GENERATED_PHASES_CONTEXT>` section. You MUST review them to ensure the universal security codes match the tech stack implemented.
- **ARCHITECTURAL PARAGRAPH EXPANSION AND FRAMEWORK DENSITY MANDATE:**
  * **STRICT NOMENCLATURE LOCKDOWN & ANTI-LOOP LAW:** Within Sections 6, 7, and 8, you ARE PERMANENTLY FORBIDDEN from repeating or ghosting the same descriptive plaintext or code mitigations (such as cross-bleeding JSX escaping or Ingress HTTP filters) across different threat models or git branch milestones. Every single generated bullet item MUST contain a minimum of 4 deep, unique integration sentences tailored strictly to that specific domain and your active framework stack dynamically extracted from the project grounding data with zero token redundancy.
