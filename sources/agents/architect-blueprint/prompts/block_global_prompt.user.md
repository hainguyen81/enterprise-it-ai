...
#}
- **GLOBAL AUTOMATIC TERMINATION BOUNDARY:** Your very first emitted token MUST be the first visible markdown header line rendered in this active User Message. The execution engine MUST continue processing the token stream continuously beyond the active segment boundary. Do NOT execute an early hard stop or immediate termination upon reaching the closing framework tag `<!--END_{{ target_segment }}-->`. You MUST smoothly transition your cursor downward to process, fully translate into Vietnamese, and explicitly emit 100% of all subsequent root sections (including Sections 6, 7, and 8) that are appended to the active template layout skeleton. Dropping, truncating, or leaving these concluding compliance sections un-translated triggers an immediate infrastructure integration failure.
...
#}
- **DYNAMIC TARGET ISOLATION LAW (HTML WRAPPER ANCHOR):** You MUST programmatically force your output generation cursor to completely skip and blind-pass 100% of this operational instruction `<RULE>` block. Identify the active anchor `<!--START_{{ target_segment }}-->` located downstream. Your very first emitted token in the response stream MUST match with absolute precision the exact text of the clean Markdown header line (starting with `#`, `##`, or `###`) located immediately AFTER that specific opening HTML framework comment tag. Zero leakage of pre-gating instruction rules, metadata words, or processing explanations is permitted before this structural header token.
...
- STEP 1 (HIGH-DENSITY DESCRIPTION): You MUST first write exactly one (1) single, cohesive technical description paragraph directly above the table initialization boundary. This paragraph MUST strictly focus on the architectural dependencies of the master components. You are ABSOLUTELY AND CRITICALLY BANNED from creating temporary sub-headers, spawning independent structural text sections (such as hallucinating text blocks titled "PHÂN PHỐI THỜI GIAN VÀ PHÂN CÔNG CÔNG VIỆC"), or listing timeline role distributions before the table opens. Leaking any raw introductory prose sections outside this single baseline paragraph triggers an immediate framework exception.
...
- You MUST systematically divide, segment, and evenly distribute the entire workload and all registered Task IDs across EXACTLY AND ONLY {{ num_phases }} distinct physical data rows. You are CRITICALLY AND ABSOLUTELY BANNED from bundling 100% of the tasks or the entire master backlog volume into a single consolidated phase row. You MUST force a strict sequential gradient progression where each individual row from Phase 1 directly up to Phase {{ num_phases }} receives a unique, proportional, and non-overlapping subset of tasks matching the progressive lifecycle defined in the `STRUCTURAL ARTIFACT PIPELINE GRADIENT` rule below. 
...
- **STRUCTURAL ARTIFACT PIPELINE GRADIENT:** You MUST distribute all backlog elements across the {{ num_phases }} phase rows based strictly on a three-tier lifecycle hierarchy:
  * Tier 1 (Core Gateways & Persistence): Foundational database storage initialization layers (`[DAT]`) and primary system endpoint gateways MUST be prioritized within the earliest available phase slots (Phase 1 and Phase 2) to build a stable workspace boundary.
...
- **PURE CHARACTER EMISSION CODE-GATE:** You ARE ABSOLUTELY BANNED from outputting any prose text, introductory sentences, or analytical commentary between the closing `</RULE>` tag and the markdown table. Your very first visible output character for this section MUST be the opening token `<!--START_PHASE_SYNOPSIS_GRID-->`, followed immediately on the next line by the literal character `|` of the table header to lock a clean layout execution.
- **IMMEDIATE MATRIX CHAR EMISSION:** You MUST transition from this rule block directly into the first markdown character of the table layout below. You ARE ABSOLUTELY BANNED from generating any intermediate text, analysis, or introductory sentences between the closing `</RULE>` tag and the opening `<!--START_PHASE_SYNOPSIS_GRID-->` anchor.
- The SynOpSis Matrix Phases table layout MUST strictly execute inside the hidden framework parsing hooks exactly as formatted below (inside the hidden HTML tags from `<!--START_PHASE_SYNOPSIS_GRID-->` to `<!--END_PHASE_SYNOPSIS_GRID-->`).
...
- **ZERO OMISSION RULE:** If a Tag ID exists in the {{ phases_and_tasks_section }} section, it MUST appear in this section `<!--START_PHASE_SYNOPSIS_GRID-->`. Truncating or omitting tags to save space is a fatal error.
- **ANTI-FENCE MARKDOWN RENDERING MANDATE:** You ARE CRITICALLY BANNED from wrapping or encapsulating the `<!--START_PHASE_SYNOPSIS_GRID-->` header, metrics, or table grid structure inside triple backticks Markdown code block fences (e.g., ```markdown or ```). You MUST output the entire table skeleton as pure raw un-fenced markdown text strings directly to the pipeline. Failure to comply will cause backend rendering truncation.
...
# STRICT OPERATIONAL AND SYNOPSIS MIRROR MANDATE FOR PHASE {{ target_phase_index }} OUT OF {{ num_phases }}:
  ...
  - **CHRONO-CUMULATIVE LEDGER VERIFICATION LAW (CORE COUNTING):** If this is the FINAL phase (Phase {{ num_phases }}), you MUST programmatically scan and audit the entire runtime history to calculate the total generated atomic sub-tasks:
      ...
      * STEP B: Count the exact number of new `<!--START_ATOMIC_SUB_TASK_NODE-->` string instances you have freshly generated in this current Phase {{ num_phases }} response block. Let this active count be integer `A`.
...
# DYNAMIC CEILING BOUNDARY ENFORCEMENT:
  - The day-by-day logs of this phase MUST strictly map to the exact day range defined for this phase from Section `<!--START_PHASE_SYNOPSIS_GRID-->` in the {{ phases_and_tasks_section }} section.
...
- **DYNAMIC PHASE ITERATION GATEKEEPER:** When evaluating this active section block for Phase {{ target_phase_index }}, you ARE CRITICALLY BANNED from dropping context or copying raw bracketed placeholders like `[Translate...]` or `[Emit...]` directly into the output stream. You MUST dynamically parse the exact matched row corresponding strictly to Phase {{ target_phase_index }} inside section `<!--START_PHASE_SYNOPSIS_GRID-->` above, extract its localized properties, and compile active operational technical data for every layout field.
...
{#
### 📈 [Translate "Phase" into the target language {{ target_language }}] {% if force_full_export %}[X]{% else %}{{ target_phase_index }}{% endif %} - [Emit the translated short phase core objective & purpose, or short summary description of this phase from Section `<!--START_PHASE_SYNOPSIS_GRID-->` in the {{ phases_and_tasks_section }} section]
#}
...
<RULE>
{#
You MUST dynamically and contextually generate a high-density, production-ready enterprise specification paragraph for each item below, fully translated into the target language context matching: {{ target_language }}. You are CRITICALLY BANNED from printing this instruction text or any raw XML rules to the output layout: Crucially, this localization mandate strictly applies to the level-3 markdown item header rows (`### 1.`, `### 2.`, etc.) as well: you MUST completely and contextually translate 100% of each item's descriptive title string into the requested target language {{ target_language }} before outputting its detailed technical description paragraph. Leaking raw un-translated English token phrases inside these section headers is a fatal violation of the global localization law. When executing this level-3 markdown header translation mandate, you MUST strictly preserve core computer science technical nomenclature and enterprise industry terms in their standard technical format. You are CRITICALLY BANNED from translating well-established technical concepts or library names into literal, non-technical prose definitions in the target language context. Specifically, you MUST leave the tokens "SQL Injection", "Cross-Site Scripting (XSS)", "Content Security Policy (CSP)", "CORS", "PII", "Capacitor Mobile Hybrid", "i18n", and "Git Branch Flow" intact as frozen technical primitives or use exact standardized industry-equivalent vocabulary in {{ target_language }}. Translating technical primitives into raw or out-of-context prose keywords will instantly break the architectural data compliance and fail the execution pipeline.
#}
You MUST dynamically and contextually translate 100% of both the level-3 markdown item header rows (`### 1.`, `### 2.`, etc.) and their underlying engineering paragraphs into the designated target language context matching: {{ target_language }}. Crucially, you are ABSOLUTELY AND PERMANENTLY BANNED from utilizing raw, literal, word-for-word machine prose translations that corrupt technical semantics (such as mapping "Injection" to non-technical consumption vocabulary or "Countermeasures" to redundant color prose adjectives). You MUST programmatically enforce an authoritative, industry-standard computer science and application security terminology mapping at runtime. Ensure that specialized concepts including "SQL Injection (SQLi)", "Cross-Site Scripting (XSS)", "Content Security Policy (CSP)", and "CORS Multi-Tenant" are rendered strictly using recognized, software-enterprise domain equivalent technical vocabulary matching the target language stream. Retain precise acronym markers intact. Leaving literal or nonsensical machine prose inside these headers will cause a fatal documentation governance failure.
...
<RULE>
{#
You MUST dynamically and contextually generate a comprehensive architectural compliance specification paragraph for each item below, fully translated into the language matching {{ target_language }}: You MUST completely translate 100% of both the level-3 markdown item header titles and their underlying operational paragraphs into {{ target_language }} seamlessly, ensuring zero raw English titles remain on the active layout. When executing this level-3 markdown header translation mandate, you MUST strictly preserve core computer science technical nomenclature and enterprise industry terms in their standard technical format. You are CRITICALLY BANNED from translating well-established technical concepts or library names into literal, non-technical prose definitions in the target language context. Specifically, you MUST leave the tokens "SQL Injection", "Cross-Site Scripting (XSS)", "Content Security Policy (CSP)", "CORS", "PII", "Capacitor Mobile Hybrid", "i18n", and "Git Branch Flow" intact as frozen technical primitives or use exact standardized industry-equivalent vocabulary in {{ target_language }}. Translating technical primitives into raw or out-of-context prose keywords will instantly break the architectural data compliance and fail the execution pipeline.
#}
You MUST dynamically and contextually translate 100% of both the level-3 markdown item header titles and their underlying operational technical compliance paragraphs into the target language context matching: {{ target_language }}. You MUST rigorously enforce professional mobile infrastructure and global internet architectural vocabulary layout conventions. You are CRITICALLY BANNED from outputting literal machine-translated prose phrases or linguistic anomalies. The complex concepts "Capacitor Mobile Hybrid Compliance Rails", "Internationalization (i18n)", and "Dynamic SEO Injection" MUST be contextually translated using official, standard software development industry equivalent terminology matching the designated language target, ensuring zero prose corruption remains on the active layout stream.
...
<RULE>
{#
You MUST dynamically and contextually generate a continuous deployment flow description paragraph for each item below, fully translated into the language matching {{ target_language }}: You MUST completely translate 100% of both the level-3 markdown integration pipeline header titles and their execution flows into {{ target_language }} seamlessly, with an absolute ban on leaking raw text titles. When executing this level-3 markdown header translation mandate, you MUST strictly preserve core computer science technical nomenclature and enterprise industry terms in their standard technical format. You are CRITICALLY BANNED from translating well-established technical concepts or library names into literal, non-technical prose definitions in the target language context. Specifically, you MUST leave the tokens "SQL Injection", "Cross-Site Scripting (XSS)", "Content Security Policy (CSP)", "CORS", "PII", "Capacitor Mobile Hybrid", "i18n", and "Git Branch Flow" intact as frozen technical primitives or use exact standardized industry-equivalent vocabulary in {{ target_language }}. Translating technical primitives into raw or out-of-context prose keywords will instantly break the architectural data compliance and fail the execution pipeline.
#}
You MUST dynamically and contextually translate 100% of both the level-3 markdown integration pipeline header titles and their continuous execution flow texts into the target language context matching: {{ target_language }}. You MUST strictly apply standard DevOps automated deployment engineering and strict CI/CD flow terminology when rendering the localized stream. Translate the milestone concepts "Daily Workspace Forking Isolation inside Git Branch Flow" and "Pipeline Guard Validation Gates" strictly into standard enterprise software delivery equivalent phrasing inside the generated headers, with an absolute ban on leaking raw text instructions or corrupted literal prose keywords.
