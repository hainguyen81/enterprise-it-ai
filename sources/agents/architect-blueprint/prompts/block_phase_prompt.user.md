...
2. **Strict Dynamic Multi-Persona Matrix Multiplication Law:** Your token engine MUST split the 4th column cell ("Architectural Component / Module Path") of the validated Phase {{ phase_idx }} row matrix from the Global Context using `<br/>` as the strict delimiter. 
  - You MUST count the total number of physical file paths extracted from that array (let this integer be F) and the total number of assigned sub-agents physically declared inside the 6th column ("Assigned Sub-Agent") of that exact same Phase row (let this integer be S). 
  - For EVERY single calendar day log unrolled, you ARE CRITICALLY AND PERMANENTLY COMMANDED to generate exactly (F * S) independent, non-truncated standalone sub-task nodes on fresh vertical rows, with each node strictly wrapped between `<!--ATOMIC_SUB_TASK_NODE_START-->` and `<!--ATOMIC_SUB_TASK_NODE_END-->`. You MUST allocate tasks exclusively to the explicit sub-agent signatures found in that row (e.g., executing Docker, GCP, and GKE sequentially for DevOps phases, and Coder, Tester, Reviewer, Doc for application phases). Skipping any registered asset or persona triggers an immediate framework integration crash.
...
4. **WORKSPACE PATH BOUNDARY & DYNAMIC TOPOLOGY CONSTRAINTS:**
    ...
    - **Verbatim Package Registry Alignment Rule:** If a file path targets a Java source or test component (.java) under the backend services layer, you ARE CRITICALLY BANNED from guessing or shortening the folder name. You MUST programmatically cross-reference the active phase with the `### 3.4. DYNAMIC MICROSERVICES TOPOLOGY REGISTRY MATRIX` table inside the Global Context. You MUST extract the exact string primitive from the "Mapped Functional Backend Packages" column (the 5th column) matching your active module tree. The folder architecture layout MUST textually match this registered package string character-by-character with 100% validation compliance (e.g., strictly preserving exactly service name such as `rate-limiting-service` or `authentication-service` folders without converting or shortening them into `auth-service`). Any shorthand leakage triggers an immediate compliance reject.
...
- **Absolute Phase Domain Semantic Isolation Law:** During the entire unrolling loop for the active Phase {{ phase_idx }}, your processing engine IS CRITICALLY AND PERMANENTLY BANNED from leaking, importing, or double-rendering any target_component paths, package descriptors, or functional definitions belonging to any foreign or adjacent phase indices. Every single physical file path string printed inside your daily sub-task nodes MUST strictly, exclusively, and character-by-character lock within the specific sub-module directory tree validated for Phase {{ phase_idx }} in the synopsis matrix row cell. Any dynamic cross-contamination or semantic drift that leaks out-of-scope assets into the active log sequence triggers an immediate pipeline isolation failure.
