...
{# START: 25/08/2026 #}
  - **Strict Partition Cap:** You ARE CRITICALLY BANNED from allocation density skew where a single phase contains > 40% of the total functional `[REQ]` tags.
...
{# START: 24/08/2026 #}
{#  - **DYNAMIC RUNTIME EVALUATION & KEY-VALUE LOOKUP LAW:** #}
...
- **MANDATORY SYMMETRIC WORKLOAD FRACTIONAL DISTRIBUTION LAW:** You MUST programmatically enforce an even and realistic allocation of tasks across the matrix rows to prevent lazy timeline compaction:
  * Calculate the local density limit: `Max_Tasks_Per_Phase = RoundUp(Matrix_Source_Tasks_Count / (num_phases - 1))`.
  * You ARE CRITICALLY BANNED from stacking core functional requirements. No single phase row from Phase 1 to Phase {{ num_phases - 1 }} is allowed to contain more than `Max_Tasks_Per_Phase` active engineering tasks from Section `<!--START_PHASE_SYNOPSIS_GRID-->` in the {{ phases_and_tasks_section }} section.
...
#}
  - **CHRONO-CUMULATIVE LEDGER VERIFICATION LAW (CORE COUNTING):** If this is the FINAL phase (Phase {{ num_phases }}), you MUST execute a non-approximate string-matching loop over the exact text buffer to prevent ledger fraud:
      * STEP A: Scan the literal raw text content of the `<HISTORIC_LEDGER_MAP>` block. Count every single occurrence of the sub-task wrapper token `<!--START_ATOMIC_SUB_TASK_NODE-->` printed in all previous chunks. Let this be integer `H`.
      * STEP B: Count every single occurrence of the sub-task wrapper token `<!--START_ATOMIC_SUB_TASK_NODE-->` generated inside the active response buffer of Phase {{ num_phases }}. Let this be integer `A`.
      * STEP C: Compute `Final_Total = H + A`. You ARE CRITICALLY BANNED from dropping the historical sum `H` or only outputting the local day count. The integer value of `Final_Total` MUST exactly equal the absolute total number of rows (excluding the summary row) inside the historical SynOpSis Matrix Phases Table from Section `<!--START_PHASE_SYNOPSIS_GRID-->` in the {{ phases_and_tasks_section }} section. You MUST inject this exact evaluated integer directly into the `TOTAL_DISCRETE_SUB_TASKS_GENERATED_IN_SECTION_5` parameter.
  - **STRICT STEP 1.1 BUFFER CHAR RECOUNT COMPLIANCE:** During the final verification pass of the response stream, your execution engine MUST run a literal, token-level counting pass strictly over the active memory text buffer. You MUST manually scan and sum every single instance of the custom HTML anchor `<!--START_ATOMIC_SUB_TASK_NODE-->` freshly printed or loaded. If the math count fails to match the value committed to `TOTAL_DISCRETE_SUB_TASKS_GENERATED_IN_SECTION_5` inside your property block, halt the stream instantly to trigger a circuit-breaker failure.
{# END: 25/08/2026 #}
...
* **[Translate "Low-Level Technical Task Instruction" into the target language {{ target_language }}]:** Output high-density technical instructions, operational validation steps, or schema parameters fully translated into the target language context, attaching explicit inline Tag IDs. **Strict Single-Line Table Invariant:** Every sentence and instruction rendered inside this specific layout field MUST be written as a single, continuous, unbroken plaintext line. You are ABSOLUTELY BANNED from emitting literal newline carriage returns (`\n`) or markdown list tokens (`-`, `*`) inside this area. If vertical spacing is structurally required for technical readability, you MUST strictly use the native HTML break tag `<br>` as the unique segment separator to safeguard table grid layout integrity.
{# END: 25/08/2026 #}
...
{# START: 24/08/2026 #}
- **FINAL CHUNK ISOLATED LEDGER REFLECTION:** When computing the value for `TOTAL_DISCRETE_SUB_TASKS_GENERATED_IN_SECTION_5`, you must natively count only the occurrences of `<!--START_ATOMIC_SUB_TASK_NODE-->` inside the active payload block. You are forbidden from forcing artificial alignment that compresses or damages downstream Section 6, 7, and 8 architectures.
...
- **ARCHITECTURAL PARAGRAPH EXPANSION AND FRAMEWORK DENSITY MANDATE:**
  ...
  * For EACH independent security countermeasure, mobile hybrid rule, and deployment gate, you MUST output a minimum of 4 distinct, continuous, and high-density technical analysis sentences fully translated into the target language.
