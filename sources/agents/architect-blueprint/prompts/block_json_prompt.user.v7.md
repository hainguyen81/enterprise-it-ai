...
- **Step 2: The Pure Algebraic Step-by-Step Exclusion Filtering Matrix**:
  * You MUST process the requested chunk range `[{{ current_start_day }}, {{ current_end_day }}]` by checking each target integer index `K` sequentially and independently (where `K` increments linearly from `{{ current_start_day }}` up to `{{ current_end_day }}`).
