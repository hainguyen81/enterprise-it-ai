  ...
  "days": [
    {
      ...
      "context_section": "Triển khai tính năng bản địa hóa ngôn ngữ và SEO đa ngôn ngữ (Anh, Việt, Tây Ban Nha)",
      "sub_tasks": [
        {
          "id": "D3_ST1",
          "agent": "Coder",
          "desc": "Cấu hình thẻ meta hreflang động cho các ngôn ngữ (Anh, Việt, Tây Ban Nha), phát hiện ngôn ngữ mặc định từ header Accept-Language và hỗ trợ chuyển đổi locale không cần tải lại trang.",
          "targeted_tags": [
            "[REQ-022]",
            "[REQ-023]",
            "[NFR-007]"
    ...
    },
    {
      "day": 4,
      "context_file": ".ai/.plan/.context/phase-4.context.blueprint.md",
      "context_section": "Triển khai phân hệ báo cáo điểm danh CSV và bảng điều khiển tóm tắt ghi danh thời gian thực",
      "sub_tasks": [
        {
          "id": "D4_ST1",
          "agent": "Coder",
          "desc": "Xây dựng các endpoint REST xuất báo cáo điểm danh định dạng CSV theo trung tâm và khoảng thời gian với các cột StudentName, CourseName, AttendanceDate, Status, cùng API tổng hợp số liệu thời gian thực cho bảng điều khiển của Center Admin. DDL: SELECT s.fullName, c.title, a.attendanceDate, 'Present' as status FROM attendance a JOIN users s ON a.studentId = s.userId JOIN courses c ON a.courseId = c.courseId WHERE a.attendanceDate BETWEEN :startDate AND :endDate; API Contract: endpoint: /api/v1/reports/attendance/csv, method: GET, query_params: centerId: uuid, startDate: date, endDate: date, response: text/csv",
          "targeted_tags": [
            "[REQ-024]",
            "[REQ-025]"
          ],
