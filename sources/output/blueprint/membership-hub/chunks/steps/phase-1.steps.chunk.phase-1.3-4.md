    ...
    * **🚨 ENTERPRISE LAYERED TOPOLOGY ENFORCEMENT LAW (Enforced for Coder, Tester, DevOps (such as Docker, GCP, GKE), and Reviewer Across All Backend and Frontend/Mobile Stacks)**: Every single persona interacting with source code implementation, unit testing, component verification, or static analysis MUST dynamically expand naked directories into precise, enterprise-standard architecture layered topologies based on the project domain:
      ...
      - **🚨 MANDATORY HARDENED PROJECT-ONLY DOCUMENTATION ARTIFACT LAW (Enforced for Agent Doc)**:
          * **For 'Doc' Agent Tasks**: The 'Doc' agent acts strictly as a Principal Technical Writer and Enterprise Systems Architect. You are ABSOLUTELY AND CRITICALLY BANNED from outputting raw directories, temporary folder paths, or system-level agent metadata contexts (e.g., paths containing `.ai/`, `.plan/`, or `.context/` such as `.ai/.plan/.context/phase-1.context.blueprint.md`).
            ...
            * **LOCALIZATION & TRANSLATION ENGINE OVERRIDE**: Keep all structural JSON keys (`phase_id`, `phase_name`, `days`, `sub_tasks`, `id`, `agent`, `desc`, `components`, `context_file`, `context_section`) permanently in raw Technical English to protect backend pipelines. However, you MUST fully translate 100% of the human-readable text values strictly *inside* the fields—specifically `phase_description`, `context_section`, and sub-task `desc`—into the target language requested by the parameters: **🇻🇳 Vietnamese**. CRITICAL: You MUST escape 100% of all interior double quotes within the translated `desc` field using the valid backend escape token (`\\\"`). No raw unescaped literal quotes are allowed to bleed into the payload. You MUST preserve raw Technical English strings only for unaccented file paths, tracing Tag IDs (`[REQ-XXX]`), and literal Sub-Agent tokens.
...
Analyze the attached Phase 1 Context Markdown content inside the `--- PHASE 1 CONTEXT MARKDOWN ---` section. 
...
- **Step 1: Strict Token Count Discovery (The Dynamic Z Boundary Lock)**:
  * You MUST physically look into `<PHASE_CONTEXT_REFERENCE_BOUND>` and count the exact frequency of the literal tag `<!--DAY_HEADER_START-->`. This dynamic total count is defined as variable `Z`. 
...
- **Step 2: The Pure Algebraic Step-by-Step Exclusion Filtering Matrix**:
  * You MUST process the requested chunk range `[3, 4]` by checking each target integer index `K` sequentially and independently (where `K` increments linearly from `3` up to `4`).
  * For each individual index `K`, execute the following strict mathematical filter before writing any JSON output:
    ...
    * **CASE B: Index In Physical Bounds (1 <= K <= Z)**:
      Only if index `K` is less than or equal to `Z` (`K <= Z`), locate the exact K-th physical `<!--DAY_HEADER_START-->` block in the markdown text. 
...
- **Step 3: The Total Range Empty Short-Circuit Execution Gate**:
  * If you evaluate every single index `K` within the range `[3, 4]` and discover that **ALL active indices fall under CASE A (K > Z)**, you MUST immediately skip the generation of the detailed daily block layout. 
...
{
  "phase_id": 1,
  "phase_name": "Khởi Tạo Khung Dự Án, Lược Đồ Dữ Liệu Hợp Nhất Và Dịch Vụ Xác Thực",
  "phase_description": "Hoàn tất nền móng hạ tầng của nền tảng membership-hub: sinh descriptor build gốc Maven multi-module cùng descriptor module con auth-service và db-migrations theo mô hình microservices Java 21/Quarkus 3.15, đồng thời khởi tạo workspace frontend Next.js/React Native với TypeScript strict mode [ARC-000]; thực thi chuỗi Flyway migration V1→V9 tạo đủ 11 bảng lõi với khóa ngoại, unique constraint và index truy vấn sub-second [DAT-001] đến [DAT-011]; triển khai endpoint POST /api/v1/auth/register hash bcrypt cost 12 cấp JWT RS256 15 phút kèm refresh token 7 ngày với cơ chế liệt kê từng trường không hợp lệ khi validation thất bại [REQ-001], [EXC-004]; đăng nhập mạng xã hội OAuth2 Firebase/Google/Facebook [REQ-002]; API gán/thay đổi vai trò người dùng kèm audit log append-only [REQ-003]. Tester bàn giao JUnit suite auth, integration test migration CSDL và profile E2E đăng ký; Doc bàn giao blueprint kiến trúc tổng thể và đặc tả tham chiếu API auth-service.",
  "project_name": "membership-hub",
  "global_context_file": ".ai/.context/membership-hub.global.blueprint.md",
  "source_target_dir": "sources/",
  "days": [
    {
      ...
      "context_section": "Hoàn Thiện Chuỗi Migration 11 Bảng Lõi Gồm Enrollments, Attendance Idempotent, StudentCards, Notifications, Promotions, Announcements, SystemSettings",
      "sub_tasks": [
        {
          "id": "D3_ST1",
          "agent": "Coder",
          "desc": "Create enrollment table with enrollment_id UUID PK, student_id UUID FK users, course_id UUID FK courses, enrollment_date TIMESTAMP NOT NULL DEFAULT now(); add UNIQUE (student_id, course_id) constraint; create indexes idx_enrollments_student_id and idx_enrollments_course_id.",
          "targeted_tags": ["[DAT-005]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V4__create_enrollments_table.sql"]
        },
        {
          "id": "D3_ST2",
          "agent": "Coder",
          "desc": "Create attendance table with attendance_id UUID PK, student_id UUID FK users, course_id UUID FK courses, attendance_date DATE NOT NULL, recorded_at TIMESTAMP NOT NULL DEFAULT now(); add UNIQUE (student_id, course_id, attendance_date) constraint; create index idx_attendance_course_date on (course_id, attendance_date).",
          "targeted_tags": ["[DAT-006]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V5__create_attendance_table.sql"]
        },
        {
          "id": "D3_ST3",
          "agent": "Coder",
          "desc": "Create student_cards table with card_id UUID PK, student_id UUID NOT NULL FK users, issue_date DATE NOT NULL, validity_days INTEGER NOT NULL CHECK > 0, remaining_days INTEGER NOT NULL DEFAULT 0; add UNIQUE (student_id) constraint; create index idx_student_cards_student_id.",
          "targeted_tags": ["[DAT-007]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V6__create_student_cards_table.sql"]
        },
        {
          "id": "D3_ST4",
          "agent": "Coder",
          "desc": "Create notifications table with notification_id UUID PK, user_id UUID FK users, group_zalo VARCHAR(100) nullable, message TEXT NOT NULL, sent_at TIMESTAMP NOT NULL DEFAULT now(), delivered BOOLEAN NOT NULL DEFAULT FALSE, retry_count SMALLINT NOT NULL DEFAULT 0, delivery_status VARCHAR(20) NOT NULL DEFAULT 'PENDING' CHECK IN ('PENDING','SENT','RETRYING','FAILED'), add indexes idx_notifications_user_id and idx_notifications_status.",
          "targeted_tags": ["[DAT-008]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V7__create_notifications_table.sql"]
        },
        {
          "id": "D3_ST5",
          "agent": "Coder",
          "desc": "Create promotions table with promo_id UUID PK, code VARCHAR(50) NOT NULL UNIQUE, discount_percent SMALLINT NOT NULL CHECK BETWEEN 1 AND 100, start_date DATE nullable, end_date DATE nullable CHECK end_date >= start_date when not null; create announcements table with announcement_id UUID PK, title VARCHAR(150) NOT NULL, content VARCHAR(2000) NOT NULL, start_date DATE nullable, end_date DATE nullable CHECK end_date >= start_date when not null; create index idx_announcements_end_date on end_date.",
          "targeted_tags": ["[DAT-009]", "[DAT-010]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V8__create_promotions_and_announcements_tables.sql"]
        },
        {
          "id": "D3_ST6",
          "agent": "Coder",
          "desc": "Create system_settings table with setting_key VARCHAR(100) NOT NULL PK, setting_value TEXT NOT NULL, description VARCHAR(255) nullable.",
          "targeted_tags": ["[DAT-011]"],
          "components": ["./sources/backend/db-migrations/src/main/resources/db/migration/V9__create_system_settings_table.sql"]
        },
        {
          "id": "D3_ST7",
          "agent": "Tester",
          "desc": "Run full migration chain V1→V9 on Testcontainers PostgreSQL; assert enrollment duplicate (student, course, date) rejected by uq_enrollments_student_course; retry_count >3 rejected by ck_notifications_retry_cap; discount_percent out of range rejected by ck_promotions_discount_range; promotion without end_date accepted as perpetual; verify information_schema shows 11 core tables exist with correct foreign key references.",
          "targeted_tags": ["[DAT-005]", "[DAT-006]", "[DAT-007]", "[DAT-008]", "[DAT-009]", "[DAT-010]", "[DAT-011]"],
          "components": ["./sources/backend/db-migrations/src/test/java/org/nlh4j/membership_hub/db/FullMigrationChainIT.java"]
        },
        {
          "id": "D3_ST8",
          "agent": "Reviewer",
          "desc": "Verify foreign key dependency order V1→V9 does not create orphan references on clean environment; confirm UNIQUE idempotent constraint on attendance (student_id, course_id, attendance_date) works; review CHECK constraints for business rule enforcement; approve merge after SonarQube clean.",
          "targeted_tags": ["[DAT-005]", "[DAT-006]", "[DAT-007]", "[DAT-008]", "[DAT-009]", "[DAT-010]", "[DAT-011]"],
          "components": []
        },
        {
          "id": "D3_ST9",
          "agent": "Doc",
          "desc": "Extend data dictionary to cover operational tables ENROLLMENTS, ATTENDANCE, STUDENT_CARDS, NOTIFICATIONS, PROMOTIONS, ANNOUNCEMENTS, SYSTEM_SETTINGS; describe columns, data types, constraints, default values; explain notification status lifecycle; detail attendance idempotent mechanism with example query; update Mermaid ER diagram; ensure consistency with core data dictionary.",
          "targeted_tags": ["[DAT-005]", "[DAT-006]", "[DAT-007]", "[DAT-008]", "[DAT-009]", "[DAT-010]", "[DAT-011]"],
          "components": ["./sources/docs/data-dictionary-operational-tables.md"]
        }
      ]
    },
    {
      "day": 4,
      "context_file": ".ai/.plan/.context/phase-1.context.blueprint.md",
      "context_section": "Triển Khai Endpoint Đăng Ký Người Dùng Hash Bcrypt Cấp JWT Và Bộ Xử Lý Ngoại Lệ Xác Thực Đầu Vào",
      "sub_tasks": [
        {
          "id": "D4_ST1",
          "agent": "Coder",
          "desc": "Define RegisterRequest record with @Email @NotBlank @Size(max=255); @NotBlank @Pattern(regexp: min 8 chars, at least one uppercase, one digit, one special char); @NotBlank @Size(max=100) for fullName; @AssertTrue for acceptedTerms; attach locale-specific messages for each constraint; ensure validation occurs in DTO, not controller.",
          "targeted_tags": ["[REQ-001]", "[EXC-004]"],
          "components": ["./sources/backend/auth-service/src/main/java/org/nlh4j/membership_hub/auth/dto/RegisterRequest.java"]
        },
        {
          "id": "D4_ST2",
          "agent": "Coder",
          "desc": "Implement UserRegistrationService with @Transactional; validate email uniqueness via prepared statement; hash password using BCrypt cost 12 to produce 60-char hash; persist Users with default role STUDENT or TEACHER based on inviteToken; set updated_at; return Users entity; all DB queries parameterized.",
          "targeted_tags": ["[REQ-001]"],
          "components": ["./sources/backend/auth-service/src/main/java/org/nlh4j/membership_hub/auth/service/UserRegistrationService.java"]
        },
        {
          "id": "D4_ST3",
          "agent": "Coder",
          "desc": "Implement JwtTokenIssuer with @Transactional; generate RS256 access token with 900s expiry containing sub, role, iss, iat, exp claims; generate opaque UUID refresh token with 604800s TTL stored in Redis key refresh:<userId>:<tokenId>; load private key from environment via MicroProfile Config; avoid hardcoding; provide verify method for downstream filter.",
          "targeted_tags": ["[REQ-001]"],
          "components": ["./sources/backend/auth-service/src/main/java/org/nlh4j/membership_hub/auth/security/JwtTokenIssuer.java"]
        },
        {
          "id": "D4_ST4",
          "agent": "Coder",
          "desc": "Implement JAX-RS resource POST /api/v1/auth/register receiving RegisterRequest via @Valid; invoke UserRegistrationService then JwtTokenIssuer; return 201 with TokenResponse (userId, email, fullName, role, accessToken, refreshToken, tokenType=Bearer); map ConstraintViolationException to 400 with invalidFields array; map EmailAlreadyExistsException to 409 with fixed message; ensure no password/hash/token logging; endpoint is public, no bearer required.",
          "targeted_tags": ["[REQ-001]"],
          "components": ["./sources/backend/auth-service/src/main/java/org/nlh4j/membership_hub/auth/resource/AuthResource.java"]
        },
        {
          "id": "D4_ST5",
          "agent": "Coder",
          "desc": "Implement GlobalExceptionMapper with @Provider for ConstraintViolationException handling; aggregate violations into invalidFields array preserving DTO order; return 400 with errorCode=AUTH_VALIDATION_FAILED; also map EmailAlreadyExistsException to 409 with fixed message; ensure no stack trace or SQL details exposed; keep constant response time.",
          "targeted_tags": ["[EXC-004]"],
          "components": ["./sources/backend/auth-service/src/main/java/org/nlh4j/membership_hub/auth/exception/GlobalExceptionMapper.java"]
        },
        {
          "id": "D4_ST6",
          "agent": "Reviewer",
          "desc": "Verify BCrypt cost >=12, token TTLs match contract (900s access, 604800s refresh); ensure no password/hash/token logging; confirm 409 response constant time to prevent enumeration; review rate limiting at gateway; approve merge after SonarQube clean.",
          "targeted_tags": ["[REQ-001]", "[EXC-004]"],
          "components": []
        },
        {
          "id": "D4_ST7",
          "agent": "Doc",
          "desc": "Update API reference chapter with OAuth2 details: list three providers (firebase, google, facebook); describe request schema (authorizationCode, redirectUri); response 200 with isNewUser flag; error 401 OAUTH2_CODE_EXCHANGE_FAILED; error 400 OAUTH2_PROVIDER_UNSUPPORTED; include Mermaid sequence diagram of popup→callback→exchange→JWT issuance; annotate security policies (state/nonce); ensure consistent linking with core data dictionary and api-auth-service-reference.md.",
          "targeted_tags": ["[REQ-002]"],
          "components": ["./sources/docs/api-auth-service-reference.md"]
        }
      ]
    }
  ]
