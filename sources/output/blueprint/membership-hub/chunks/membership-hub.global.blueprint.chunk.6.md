...
# STRICT OPERATIONAL AND SYNOPSIS MIRROR MANDATE FOR PHASE 3 OUT OF 5:
  - OPERATIONAL SCOPE: You are now executing target segment 'PART_2_PHASE_LOOP' exclusively for Phase 3 out of 5.
...
# DYNAMIC CEILING BOUNDARY ENFORCEMENT:
  ...
  - **STRICT PHASE INDEX COUPLING MANDATE:** You ARE STRICTLY FORBIDDEN from generating any text, sub-headers, logs, or sub-task blocks for other phases. You MUST contextually freeze your execution cursor and apply a hard system stop the exact microsecond you complete the output buffer wrapper for Phase 3.
...
- **DYNAMIC PHASE ITERATION GATEKEEPER:** When evaluating this active section block for Phase 3, you ARE CRITICALLY BANNED from dropping context or copying raw bracketed placeholders like `[Translate...]` or `[Emit...]` directly into the output stream. You MUST dynamically parse the exact matched row corresponding strictly to Phase 3 inside section `<!--START_PHASE_SYNOPSIS_GRID-->` above, extract its localized properties, and compile active operational technical data for every layout field.
...
- **ANTI-FENCE MARKDOWN RENDERING MANDATE:** You ARE CRITICALLY BANNED from wrapping or encapsulating the Phase 3 header, metrics, or table grid structure inside triple backticks Markdown code block fences (e.g., ` ```markdown ` or ` ``` `). You MUST output the entire phase 3 skeleton as pure raw un-fenced markdown text strings directly to the pipeline. Failure to comply will cause backend rendering truncation.
...
#### 📅 [Translate "Chronological Day-by-Day Sub-Agent Task Distribution Logs (Phase 3)" into the target language 🇻🇳 Vietnamese]
