...
- **GLOBAL AUTOMATIC TERMINATION BOUNDARY:** Your very first emitted token MUST be the first visible markdown header line rendered in this active User Message. The exact microsecond you finish printing the last visible data row or string before the active segment's closing HTML framework tag (e.g., `<!--END_...-->`, example: `<!--END_PART_2_PHASE_LOOP-->`), you MUST trigger an immediate hard stop and terminate the response stream instantly.
...
# STRICT OPERATIONAL AND SYNOPSIS MIRROR MANDATE FOR PHASE 1 OUT OF 5:
  - OPERATIONAL SCOPE: You are now executing target segment 'PART_2_PHASE_LOOP' exclusively for Phase 1 out of 5.
...
# DYNAMIC CEILING BOUNDARY ENFORCEMENT:
  ...
  - **STRICT PHASE INDEX COUPLING MANDATE:** You ARE STRICTLY FORBIDDEN from generating any text, sub-headers, logs, or sub-task blocks for other phases. You MUST contextually freeze your execution cursor and apply a hard system stop the exact microsecond you complete the output buffer wrapper for Phase 1.
  - **TARGETED SINGLE-PHASE ISOLATION RAIL:** Your entire response stream MUST focus exclusively on the requirements, tasks, components, and tag identifiers allocated to Phase 1. 
- **DYNAMIC PHASE ITERATION GATEKEEPER:** When evaluating this active section block for Phase 1, you ARE CRITICALLY BANNED from dropping context or copying raw bracketed placeholders like `[Translate...]` or `[Emit...]` directly into the output stream. You MUST dynamically parse the exact matched row corresponding strictly to Phase 1 inside section `<!--START_PHASE_SYNOPSIS_GRID-->` above, extract its localized properties, and compile active operational technical data for every layout field.
...
- **STRICT LOOP PARTITION ISOLATION LAW:** When compiling the daily logs for Phase 1, you ARE CRITICALLY BANNED from replicating, cloning, or copying task descriptions, file paths, or titles from other phases. You MUST explicitly map and unroll only the unique engineering deliverables and task indices allocated strictly to that specific Phase 1 inside the `--- BACKLOG TASKS ---` section.
...
- **ANTI-FENCE MARKDOWN RENDERING MANDATE:** You ARE CRITICALLY BANNED from wrapping or encapsulating the Phase 1 header, metrics, or table grid structure inside triple backticks Markdown code block fences (e.g., ` ```markdown ` or ` ``` `). You MUST output the entire phase 1 skeleton as pure raw un-fenced markdown text strings directly to the pipeline. Failure to comply will cause backend rendering truncation.

</COMMAND>

<!--START_PHASE_INDEX-->

### 📈 [Translate "Phase" into the target language 🇻🇳 Vietnamese] 1 - [Dynamically compute and emit a concise, high-level technical name for this milestone based on its core delivery component, completely translated into "🇻🇳 Vietnamese"]
...
- **[Translate "Target Physical Directory Matrix Map" into the target language 🇻🇳 Vietnamese]:** List all specific file paths underneath `./sources/` initialized or modified in this phase. Every single line path generated MUST be appended with its tracking Tag IDs inline.
...
#### 📅 [Translate "Chronological Day-by-Day Sub-Agent Task Distribution Logs (Phase 1)" into the target language 🇻🇳 Vietnamese]
...
<RULE>
* **[Translate "Sub-Agent Workflow Specialization" into the target language 🇻🇳 Vietnamese]:**
<RULE>
...
* **[Translate "Targeted Tag IDs" into the target language 🇻🇳 Vietnamese]:**
<RULE>
Write each baseline tracking tag out individually separated by commas, ensuring 100% coverage, e.g., [REQ-001], [DAT-002], [EXC-001].
</RULE>
* **[Translate "Target Component file path" into the target language 🇻🇳 Vietnamese] (target_component):**
<RULE>
...
* **[Translate "Low-Level Technical Task Instruction" into the target language 🇻🇳 Vietnamese]:**
<RULE>
