...
6. **Language Compliance & Technical Syntax Isolation:** You MUST generate the descriptive text report, day objectives, table structures, and "Low-Level Technical Task Instructions" strictly in the dynamic language specified by the runtime variable: **🇺🇸 English**. This mandatory requirement strictly overrides any default freezing rules for high-level timeline elements: you MUST contextually and naturally translate 100% of the uppercase and lowercase chronological milestones (specifically including all Phase and Day indicator strings) into the target output text stream matching **🇺🇸 English**. Any header line representing a phase or day milestone MUST be fully localized. Leaking the raw un-translated English tokens "PHASE" or "DAY" directly into the final markdown report headers is a fatal violation of the localization law.
...
#### 🚨 CRITICAL FULL TRANSLATION MANDATE
  - The target generation language for all human-readable outputs is permanently bound to: 🇺🇸 English. Everything MUST be translated into 🇺🇸 English, except for the explicit Technical English core tokens protected by system mandates.
...
#### 🚨 DYNAMIC INTERNATIONALIZATION & TRANSLATION ENGINE
  - Target Output Language Context: 🇺🇸 English
...
| Item | Details |
| :--- | :--- |
| **Blueprint ID** | ARCH-20260818155231 |
| **Project Name** | membership-hub |
| **Version** | 1.0 (Baseline) |
| **Date.Time** | 2026/08/18 15:52:31 |
...
- You MUST render 100% of your newly generated sentences in the designated target language: 🇺🇸 English.
</RULE>
...
- Every bullet point must be a short, punchy technical baseline statement delivering raw architectural metrics in the designated target language: 🇺🇸 English.
</RULE>
...
- **REAL-DATA COMPLIANCE ANCHOR:** You MUST extract and generate the markdown bulleted checklist based STRICTLY AND ONLY on the actual, real-world security and infrastructure data present in the raw input requirements database.
  * ANTI-HALLUCINATION RAIL: You ARE ABSOLUTELY BANNED from fabricating, looping, or generating generic administrative placeholder bullets (e.g., do NOT generate repeated lines about managing finance, HR, projects, or quality). If the source data provides fewer than 5 compliance metrics, stop immediately at the last real item. Padding out the text stream with semantic junk will trigger an immediate compiler crash.
- Every bullet point must be a short, punchy technical statement delivering raw architectural metrics in the designated target language: 🇺🇸 English.
...
Based on the provided requirements, the physical directory file path masks will be dynamically matched to the active system topology. The directory delimiter will be the Unix forward-slash `/` character.

#### Backend Infrastructure Core Stack:

- `./sources/backend/membership-hub/`
  - Sub-modules:
    - `./sources/backend/membership-hub/services/`
    - `./sources/backend/membership-hub/database/`
    - `./sources/backend/membership-hub/tests/`

#### Frontend & Cross-Platform UI Mobile Stack:

- `./sources/frontend/membership-hub/`
  - Sub-modules:
    - `./sources/frontend/membership-hub/components/`
    - `./sources/frontend/membership-hub/views/`
    - `./sources/frontend/membership-hub/mobile/`

#### DevOps Infrastructure Logic:

- `./sources/infra/membership-hub/`
  - Sub-modules:
    - `./sources/infra/membership-hub/deployment/`
    - `./sources/infra/membership-hub/docker/`
    - `./sources/infra/membership-hub/gke/`

#### Document Asserts:

- `./sources/docs/membership-h
