...
| Item | Details |
| :--- | :--- |
| **Blueprint ID** | ARCH-20260812134837 |
| **Project Name** | membership-hub |
| **Version** | 1.0 (Baseline) |
| **Date.Time** | 2026/08/12 13:48:37 |
...
### 1.1. Core System Modality & Architecture Modality
- Hệ thống được thiết kế theo mô hình đa trung tâm với các thành phần chính bao gồm quản lý thành viên, theo dõi điểm danh, quản lý khóa học và giao tiếp đa kênh.
- Sử dụng kiến trúc microservices với các dịch vụ độc lập cho mỗi chức năng chính.
- Triển khai mô hình Role-Based Access Control (RBAC) để quản lý quyền truy cập theo vai trò người dùng.
- Sử dụng JWT cho xác thực và phân quyền, với token truy cập hết hạn sau 15 phút và token làm mới sau 7 ngày.
- Hệ thống bao gồm các thành phần chính: quản lý người dùng, quản lý trung tâm, quản lý khóa học, quản lý học viên, quản lý điểm danh, quản lý thẻ thành viên và quản lý thông báo.

### 1.2. Enterprise Data Flow Topologies & Core Ecosystems
- Hệ thống sử dụng kiến trúc event-driven với các kênh thông báo đa kênh bao gồm push notifications và Zalo groups.
- Dữ liệu được lưu trữ trong PostgreSQL với các bảng chính bao gồm Users, Centers, Courses, Enrollments, Attendance, StudentCards, Notifications, Roles, Promotions và Announcements.
- Sử dụng Apache Kafka cho xử lý các sự kiện như điểm danh, đăng ký khóa học và thông báo.
- Hệ thống bao gồm các dịch vụ chính: Authentication Service, User Management Service, Center Management Service, Course Management Service, Enrollment Service, Attendance Service, Student Card Service, Notification Service, Promotion Service và Announcement Service.
- Các dịch vụ giao tiếp với nhau thông qua REST APIs và event streams.
- Hệ thống sử dụng Firebase Cloud Messaging (FCM) và APNs cho push notifications trên thiết bị di động.
- Sử dụng Google Kubernetes Engine (GKE) cho triển khai và quản lý các dịch vụ trong môi trường cloud.

## 📁 2. TECH STACK DEPENDENCIES & ECOSYSTEM LIBRARIES
- **Backend Infrastructure Core Stack:** Quarkus 3.8.2, PostgreSQL 15.4, Apache Kafka 3.6.0, Firebase Admin SDK 12.0.0, JWT 0.11.5
- **Frontend & Cross-Platform UI Mobile Stack:** Next.js 14.1.0, React 18.2.0, React Native 0.73.0, Tailwind CSS 3.3.0
...
## 📁 3. GLOBAL GUARDRAILS & ENTERPRISE COMPLIANCE STANDARDS
- Tất cả các đường dẫn phải bắt đầu với `./sources/`.
- Các gói Java phải nằm trong `org.nlh4j.saas.membershiphub`.
- Các đường dẫn được kiểm thử phải được định dạng theo cú pháp `<source_component_or_token>;<test_suite_file_to_execute>`.
